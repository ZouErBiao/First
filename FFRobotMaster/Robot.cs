...
namespace FFRobotMaster
{
    ...
    public class Robot
    {
        ...
        /// 这个机器人的原始数模。
        /// </summary>
        //private XModel RobotModel0;
        private XFile RobotModel;
        /// <summary>
        /// 将原始数模应用了动作的输出数模。
        /// </summary>
        //public XModel UpdatedModel;
        ...
        public void Update()
        {
            ...
            for (int t = 0; t < AllBones.Count; t++)
            {
                ...
                {
                    tmpsrc.Seek(ar[AllBones[t].Subset].VertexStart * PositionNormalTextured.Length(), System.IO.SeekOrigin.Begin);
                    tmpdest.Seek(ar[AllBones[t].Subset].VertexStart * PositionNormalTextured.Length(), System.IO.SeekOrigin.Begin);
                    if (AllBones[t].myUpdatedPosition.Y < sumy)
                        for (int tt = 0; tt < ar[AllBones[t].Subset].VertexCount; tt++)
                        {
                            PositionNormalTextured pnt = tmpsrc.Read<PositionNormalTextured>();
                            PositionNormalTextured pnt2 = new PositionNormalTextured(Vector3.Transform(pnt.Position, tmpm[t]).ToVector3(),
                                Vector3.TransformNormal(pnt.Normal, tmpm[t]),
                                pnt.UV);
                            tmpy = Math.Min(tmpy, pnt2.Position.Y);
                            tmpdest.Write<PositionNormalTextured>(pnt2);
                        }
                    else
                        for (int tt = 0; tt < ar[AllBones[t].Subset].VertexCount; tt++)
                        {
        ...
        {
            //RobotModel0 = new XModel(modelfn);
            RobotModel = new XFile(modelfn);
            //UpdatedModel = new XModel(modelfn);
