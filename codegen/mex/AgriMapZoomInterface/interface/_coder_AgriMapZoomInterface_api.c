 ...
 *
 * Code generation for function '_coder_AgriMapZoomInterface_api'
 *
 */
...
static real_T e_emlrt_marshallIn(const emlrtStack *sp, const mxArray *u, const
  emlrtMsgIdentifier *parentId);
static const mxArray *e_emlrt_marshallOut(const struct4_T *u);
...
static const mxArray *d_emlrt_marshallOut(const struct3_T u)
{
  ...
  emlrtAddField(y, c_emlrt_marshallOut(u.error), "error", 0);
  emlrtAddField(y, c_emlrt_marshallOut(u.warn), "warn", 0);
  return y;
}
...
static const mxArray *e_emlrt_marshallOut(const struct4_T *u)
{
  ...
  const mxArray *m3;
  const mxArray *c_y;
  static const int32_T iv47[2] = { 1, 100 };

  real_T *pData;
  int32_T i;
  const mxArray *d_y;
  static const int32_T iv48[2] = { 1, 100 };
  ...
  *(uint8_T *)mxGetData(m3) = u->mapCnt;
  emlrtAssign(&b_y, m3);
  emlrtAddField(y, b_y, "mapCnt", 0);
  c_y = NULL;
  m3 = emlrtCreateNumericArray(2, iv47, mxDOUBLE_CLASS, mxREAL);
  ...
  d_y = NULL;
  m3 = emlrtCreateNumericArray(2, iv48, mxDOUBLE_CLASS, mxREAL);
  pData = (real_T *)mxGetPr(m3);
  for (i = 0; i < 100; i++) {
    pData[i] = u->mapLon[i];
  }
  ...
  e_y = NULL;
  m3 = emlrtCreateNumericArray(2, iv49, mxDOUBLE_CLASS, mxREAL);
  pData = (real_T *)mxGetPr(m3);
  for (i = 0; i < 100; i++) {
    pData[i] = u->mapAlt[i];
  }
  ...
  emlrtMsgIdentifier *msgId, real_T ret[100])
{
  ...
  for (i35 = 0; i35 < 100; i35++) {
    ret[i35] = (*(real_T (*)[100])mxGetData(src))[i35];
  }
  ...
  plhs[3])
{
  ...
  emlrt_marshallIn(&st, emlrtAliasP(prhs[0]), "mapB", &mapB);
  lineM = f_emlrt_marshallIn(&st, emlrtAliasP(prhs[1]), "lineM");
  params = h_emlrt_marshallIn(&st, emlrtAliasP(prhs[2]), "params");

  /* Invoke the target function */
  AgriMapZoomInterface(&st, &mapB, &lineM, &params, &ver, &out, &mapOut);

  /* Marshall function outputs */
  plhs[0] = c_emlrt_marshallOut(ver);
  plhs[1] = d_emlrt_marshallOut(out);
  plhs[2] = e_emlrt_marshallOut(&mapOut);
}

/* End of code generation (_coder_AgriMapZoomInterface_api.c) */
