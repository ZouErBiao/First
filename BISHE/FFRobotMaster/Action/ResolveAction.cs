...
namespace FFRobotMaster
{
    public class ResolveAction
    {
        ...
        public List<Frame> KeyFrames = new List<Frame>(500);
            ...
            while ((strline = stream.ReadLine()) != null)//是否结束
            {
                ...
                else if (head[0] == "N")
                {
                    ...
                    for (int a=0;a<this.fnum;a++)
                    {
                        ...
                        for (int i = 0; i < 17; i++)
                        {
                            frame[i] = (float)Math.Floor(Math.Abs((float.Parse(headd[i + 1]))*270f/4095));
                            //frame[i] = float.Parse(headd[i + 1]) + float.Parse(zeroPosition[i]);
                        }
                        frame.Pause = int.Parse(headd[19]);
